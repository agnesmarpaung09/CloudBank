...
const Testimonial = () => {
  ...
  return (
    <div className="grid grid-cols-1 items-center mt-28 min-h-screen">
      <Slider {...sliderSettings}>
        {dataTestimoni.map((row, index) => (
          <div key={index} className="flex items-center mb-32">
            <div className="xl:flex bg-white rounded-2xl shadow-2xl xl:w-auto xl:h-60 mr-10 sm:mr-8 md:mr-16 lg:mr-28 xl:mr-8 2xl:mr-12 items-center">
              <div className="xl:w-1/3 bg-center rounded-xl">
                <Image
                  src={row.image}
              ...
              <div className="xl:w-2/3 p-4 xl:p-0 mx-2 xl:mx-6 2xl:p-4">
                <h2 className="text-lg sm:text-md font-semibold mt-2">{row.name}</h2>
                <p className="text-sm sm:text-xs mt-2">{row.busines}</p>
                <p className="text-md sm:text-sm mt-2 text-justify">{row.description}</p>
              </div>
            </div>
          </div>
        ))}
